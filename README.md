...
1. Differences between using _sessions_ or _JSON Web Tokens_ for authentication.
   With sessions, user data is stored in cookies and the server is responsible for keeping track of those cookies.
   With JWT, the server provides the client with a jwt upon successful login, that authenticates and authorizes (if valid) the user's requests.
2. What does `bcrypt` do to help us store passwords in a secure manner.
   It hashes the password using a one-way hashing algorithm to prevent hackers from easily obtaining the password.
3. How are unit tests different from integration and end-to-end testing.
   Unit tests test a sepcific unit of code. While integration usually tests more than one and make sure they work well together.
4. How _Test Driven Development_ changes the way we write applications and tests.
   With TDD, tests are usually written first and the code is created to make sure the tests pass.
